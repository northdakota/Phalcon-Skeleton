...
Методы контроллера имеют синтаксис:
```
public function имяAction()
{
}
```
Что-либо еще не обязательно. По имени контроллера и метода будет самостоятельно определена папка  и имя файла шаблона.
Если необходимо передать переменную в шаблон - ```$this->view->переменная = значение;```
### <a name="Redirects"></a>Редиректы

##### Для редиректов есть два относительно стабильных способа:
1. Диспатчером
```
...
```
Аккуратно. Это фактически не редирект, а передача управления. В переменной $_POST все еще сохранены значения, что может привести к веселым багам. Например, к нежелательной фильтрации, по сохраняемым полям.
...
Если вы используете еще один способ редиректа - то вы однозначно извращенец!
...
Самый большой недостаток фалькорна - его нельзя пропатчить.